# Submission.
# O(kn) time complexity and O(k + n) space complexity.
# Where `n` is array length and `k`` is shortest word length.
...
- Revisit zipping.
- Space complexity O(k) for common-prefix letters, and strictly, O(1) for the set constructor, as only 26 lowercase alphabetic letters, not O(n).
...
- The time complexity is O(n), and O(n + k) space, with O(n) for the unpacking of O(n) pointers in zip, then O(k) for final slice.
"""

# Pythonic solution.
# O(kn) time and O(n + k) space.
